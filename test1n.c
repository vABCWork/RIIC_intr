...
void main(void)
{
	...
	test_riic_intr_aht25();		//  温湿度センサ(AHT25)のテスト  (RIIC割り込み使用)
	

//	test_riic_intr_thermo_pile();		// サーモパイルのテスト(RIIC割り込み使用)

}


// 温湿度センサ(AHT25)のテスト  (RIIC割り込み使用)
//  
	...
	while(1)			
	{
	  ...
	  if ( timer_10msec_cnt == 0 ) {   // 最初 		
	     if ( flg_task0 == 0 ) {
	           ...
	           wr_sensor_cmd();	//温湿度センサへの測定開始コマンド送信
	       ...
	       if ( flg_task1 == 0 ) {    
		    ...
		    rd_sensor_humi_temp();	// 温湿度データの読み出し  
	...
	while(1)			
	{
	  ...
	  if ( timer_10msec_cnt == 0 ) {   // 最初 		
	     if ( flg_task0 == 0 ) {
		   ...
		   rd_thermo_pile(0);          // センサの周囲温度(Self temperature)(TA)を読み出す
		  ...
		  flg_task1 = 1;   
	       ...
	       if ( flg_task2 == 0 ) {    
		    ...
		    rd_thermo_pile(1);          // 測定対象物の温度(Objet temperature)(TO)を読み出す
		...
		if ( flg_task3 == 0 ) {   
		  ...
		  Cal_Ta_To_temperature();	// 測定対象物の温度の計算
